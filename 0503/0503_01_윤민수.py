...
city[-4] = '서울특별시'
print("city : ", city)
# print("city[5] : ", city[5])        # 리스트의 6번쨰 요소 값 출력 -> 오류 발생
                                      # IndexError: list index out of range
'''
2. 슬라이싱(slicing) : 시퀀스 자료형에서 일부를 잘라내어
                        동일한 자료형으로 반환하는 기능.
[start : stop : step] => start에서 시작해서 stop 이전가지 step 간격으로 추출
'''
...
# in 연산자는 for 빈복문에 효율적을 사용.
for i in text3 :   # text3 안에 있는 문자를 하나씩 i변수에 넣어서 반복.
    print(i)
...
'''
# 문자열 text2의 길이
print("text2의 길이 : ", len(text2))    # 22 => 띄어쓰기 포함.
# 리스트 city의 길이
